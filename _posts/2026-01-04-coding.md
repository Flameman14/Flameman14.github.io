---
title: 대외활동용 코드 작성   # 글 제목
date: 2026-01-04 21:21:00 +0900 # 작성 날짜 (형식 지킬 것!)
categories: [바이브코딩, 대외활동]        # [대분류, 소분류] (대분류만 써도 됨)
tags: [대외활동, 바이브코딩, 스프레드시트]     # 태그 (소문자로 쓰는 게 좋음)
---
<br>

## 만들게 된 계기
대외활동에서 공모전을 열었습니다. \
많은 신청자들이 들어와 수기 작성이나 기억력만 믿기에는 누락의 위험과 정보 공유의 한계가 있을 것 같아 자동화 시스템을 만들고자 하였습니다. <br><br>

### 제출 신청서 분류 시트

첫번째로 만들고자 한 시스템은 스프레드시트를 활용하여 문서를 두 종류로 나누는 것입니다. \
승인 시스템이라고 생각하면 됩니다.\
간단한 방식이라 그다지 수정하지 않았습니다.  <br>

기존에는 버튼을 클릭하여 파일을 가져오고 배분 후 수동으로 정리하는 시스템이었으나, 트리거 기능을 활용하여 화면이 열릴 시 자동으로 불러오도록 방식을 바꾸었습니다. <br> <br>



```markdown
## 최신 버전 (v2.0)
지금 바로 사용 가능한 코드입니다.

```javascript
// =======================================================
// [설정 1] 폴더 ID 설정
var SOURCE_FOLDER_ID = '미분류 파일 폴더 ID'; 
var TARGET_FOLDER_A_ID = '아이디어 제출 파일 폴더 ID';
var TARGET_FOLDER_B_ID = '제출 및 실행 파일 폴더 ID';

// [설정 2] 평가표 시트 ID 설정 (여기에 복사한 ID를 넣으세요!)
var EVALUATION_SHEET_ID = '분류한 파일을 평가표 시트로 이동하기 위한 부분'; 
// =======================================================

function onOpen() {
  var ui = SpreadsheetApp.getUi();
  ui.createMenu('📂 파일 정리 도구')
    .addItem('1. 미분류 파일 불러오기', 'loadUnsortedFiles')
    .addItem('2. 선택한대로 이동 & 평가표 등록', 'moveFilesBasedOnSelection')
    .addSeparator()
    .addItem('3. (수동) 평가표 동기화', 'syncToEvaluationSheet')
    .addToUi();
}

// 1. 미분류 파일 불러오기 (기존 동일)
function loadUnsortedFiles() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
  cleanUpEmptyRows(sheet); // 빈 줄 정리

  var folder = DriveApp.getFolderById(SOURCE_FOLDER_ID);
  var files = folder.getFiles();
  
  var lastRow = sheet.getLastRow();
  var existingIds = [];
  if (lastRow > 1) {
    var data = sheet.getRange(2, 5, lastRow - 1, 1).getValues(); 
    existingIds = data.map(function(row) { return row[0]; });
  }

  var newCount = 0;
  // 분류 선택 드롭다운
  var rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(['아이디어 제출', '제출 및 실행'], true).setAllowInvalid(false).build();

  while (files.hasNext()) {
    var file = files.next();
    var fileId = file.getId();
    
    if (existingIds.indexOf(fileId) === -1) {
      sheet.appendRow([file.getName(), file.getUrl(), "", "대기", fileId]);
      sheet.getRange(sheet.getLastRow(), 3).setDataValidation(rule);
      newCount++;
    }
  }
  
  var msg = newCount > 0 ? newCount + '개의 파일을 불러왔습니다.' : '새로운 파일이 없습니다.';
  SpreadsheetApp.getUi().alert(msg);
}

// 빈 줄 정리 함수
function cleanUpEmptyRows(sheet) {
  var lastRow = sheet.getLastRow();
  for (var i = lastRow; i >= 2; i--) {
    var rowData = sheet.getRange(i, 1, 1, 5).getValues()[0];
    if (rowData[0] === "" && rowData[4] === "") {
      sheet.deleteRow(i);
    }
  }
}

// 2. 파일 이동 + 평가표 등록까지 한 번에 실행
function moveFilesBasedOnSelection() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
  var data = sheet.getDataRange().getValues(); 
  var folderA = DriveApp.getFolderById(TARGET_FOLDER_A_ID);
  var folderB = DriveApp.getFolderById(TARGET_FOLDER_B_ID);
  
  var movedCount = 0;

  for (var i = 1; i < data.length; i++) {
    var category = data[i][2]; 
    var fileId = data[i][4];   
    
    if (category != "" && fileId != "") {
      try {
        var file = DriveApp.getFileById(fileId);
        
        if (category == "아이디어 제출") {
          file.moveTo(folderA);
          sheet.getRange(i + 1, 4).setValue("이동완료"); 
          sheet.getRange(i + 1, 1, 1, 5).setBackground("#FFF2CC"); 
          movedCount++;
        } else if (category == "제출 및 실행") {
          file.moveTo(folderB);
          sheet.getRange(i + 1, 4).setValue("이동완료");
          sheet.getRange(i + 1, 1, 1, 5).setBackground("#D9EAD3"); 
          movedCount++;
        }
      } catch (e) {
        sheet.getRange(i + 1, 4).setValue("에러: " + e.message);
      }
    }
  }
  
  if (movedCount > 0) {
    // 이동이 끝났으면 평가표로 정보 전송
    syncToEvaluationSheet(); 
    SpreadsheetApp.getUi().alert('파일 이동 및 평가표 등록이 완료되었습니다!');
  } else {
    SpreadsheetApp.getUi().alert('이동할 파일이 없습니다.');
  }
}

// 3. [핵심] 평가표 시트에 자동으로 내용 추가 및 서식 적용 함수
function syncToEvaluationSheet() {
  var sourceSheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
  var sourceData = sourceSheet.getDataRange().getValues();
  
  // 평가표 파일 열기
  try {
    var targetSpreadsheet = SpreadsheetApp.openById(EVALUATION_SHEET_ID);
  } catch (e) {
    SpreadsheetApp.getUi().alert('평가표 시트 ID가 잘못되었습니다. 스크립트 맨 위 설정을 확인해주세요.');
    return;
  }
  
  var targetSheet = targetSpreadsheet.getSheets()[0]; // 첫 번째 시트 사용
  var targetHeaders = targetSheet.getRange("1:1").getValues()[0]; // 1행(헤더) 가져오기
  
  var addedCount = 0;

  // 소스 데이터 스캔 (2행부터)
  for (var i = 1; i < sourceData.length; i++) {
    var fileName = sourceData[i][0]; // A열: 파일 이름
    var fileUrl = sourceData[i][1];  // B열: 링크
    var status = sourceData[i][3];   // D열: 상태
    
    // '이동완료' 상태인 파일만 처리
    if (status === "이동완료") {
      // 이미 평가표에 있는지 이름으로 확인 (중복 방지)
      // 정확한 매칭을 위해 파일 이름이 1행에 있는지 확인
      var exists = false;
      for (var k = 0; k < targetHeaders.length; k++) {
        if (targetHeaders[k] === fileName) { // 이름이 똑같으면 이미 있는 것으로 간주
          exists = true;
          break;
        }
      }
      
      // 평가표에 없으면 새로 추가
      if (!exists) {
        var newCol = targetSheet.getLastColumn() + 1; // 새로운 열 번호
        
        // 1행에 [링크가 걸린 이름] 넣기
        var formula = '=HYPERLINK("' + fileUrl + '", "' + fileName + '")';
        targetSheet.getRange(1, newCol).setFormula(formula);
        
        // ★ 평가표 서식 및 드롭다운 자동 적용 (원격 제어) ★
        applyEvaluationFormat(targetSheet, newCol);
        
        addedCount++;
        // 헤더 배열 업데이트 (방금 추가한 것도 중복 체크에 반영)
        targetHeaders.push(fileName);
      }
    }
  }
}

// 평가표 서식 적용 함수 (이전에 만든 깔끔한 버전)
function applyEvaluationFormat(sheet, col) {
  var dropdownRows = [3, 4, 5, 6, 9, 10, 11, 14, 15, 17, 20, 21];
  var totalRow = 19;

  // 드롭다운 (O/X)
  var rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(['O', 'X'], true).setAllowInvalid(false).build();
  
  dropdownRows.forEach(function(row) {
    sheet.getRange(row, col).setDataValidation(rule);
  });

  // 총점 수식 (=COUNTIF)
  var colLetter = getColumnLetter(col);
  var formulaStr = '=COUNTIF(' + colLetter + '3:' + colLetter + '17, "O")';
  sheet.getRange(totalRow, col).setFormula(formulaStr);

  // 디자인 (총점만 노란색)
  sheet.getRange(totalRow, col)
       .setBackground("#FFF2CC")
       .setFontWeight("bold")
       .setHorizontalAlignment("center");
       
  // 전체 가운데 정렬 (선택사항)
  sheet.getRange(3, col, 19, 1).setHorizontalAlignment("center");
}

// 열 번호 -> 알파벳 변환
function getColumnLetter(colIndex) {
  var temp, letter = '';
  while (colIndex > 0) {
    temp = (colIndex - 1) % 26;
    letter = String.fromCharCode(temp + 65) + letter;
    colIndex = (colIndex - temp - 1) / 26;
  }
  return letter;
}
```
<br><br>

### 평가표 시트 

평가표 시트는 여러번 수정하였습니다.\
처음에는 단순히 평가만 하는 시트로 사용했지만, 이후 개인 평가 및 팀 전원 평가, 최종 승인을 위한 보고서 역할도 수행하기 위해 수정에 수정을 거듭하였습니다. \
크게 3가지 영역으로 구분되어 있습니다. <br><br>


<details markdown="1">
<summary>1. 초기 버전 (v1.0)</summary>
  
<br>

처음에는 단순한 평가용 시트였습니다. 제출 신청서 분류 시트에서 파일 정보를 가져오고, 평가를 위해 O, X 드롭다운과 합산 기능을 자동 생성하도록 하였습니다.


```javascript

function updateEvaluationListCountO() {
  // ==========================================
  // 1. 기본 설정 (사용자 환경에 맞게 수정)
  // ==========================================
  var FOLDER_ID = "아이디어 제출 폴더ID"; // 프로젝트 파일이 있는 구글 드라이브 폴더 ID
  var targetSheets = ["평가자1", "평가자2", "평가자3"];   // 평가를 진행할 시트 이름 목록
  
  // ▼ 이미지 기준 'O' 체크가 필요한 행 번호 (총 10개 항목)
  // 공익성(3,4,5,6) + 실현가능성(10,11,12) + 창의성(16,17) + 지속가능성(20)
  var ROWS_TO_CHECK = [3, 4, 5, 6, 10, 11, 12, 16, 17, 20]; 

  // ==========================================
  // 2. 메인 로직 시작
  // ==========================================
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var folder, files;

  try {
    folder = DriveApp.getFolderById(FOLDER_ID);
    files = folder.getFiles();
  } catch (e) {
    ss.toast("❌ 폴더 ID를 확인해주세요. 폴더를 찾을 수 없습니다.");
    return;
  }
  
  // 드라이브 파일 목록 가져오기
  var fileList = [];
  while (files.hasNext()) {
    var file = files.next();
    fileList.push({ name: file.getName(), url: file.getUrl() });
  }

  if (fileList.length === 0) {
    ss.toast("📂 지정된 폴더에 파일이 없습니다.");
    return;
  }

  // 각 시트(평가자) 별로 순회
  targetSheets.forEach(function(sheetName) {
    var sheet = ss.getSheetByName(sheetName);
    if (!sheet) return; // 해당 이름의 시트가 없으면 건너뜀

    // 시트의 마지막 행 계산 (최소 30행 확보)
    var lastRow = sheet.getLastRow();
    if (lastRow < 30) lastRow = 30; 

    // 기존에 이미 추가된 팀 확인 (중복 추가 방지)
    var lastCol = sheet.getLastColumn();
    var existingTeams = [];
    if (lastCol > 1) {
      // 1행(헤더)에 있는 팀명들을 가져옴
      existingTeams = sheet.getRange(1, 2, 1, lastCol - 1).getValues()[0]; 
    }

    // A열을 스캔하여 '총합' 행과 '발표' 행의 위치를 동적으로 찾음
    var colA = sheet.getRange(1, 1, lastRow, 1).getValues(); 
    var rowTotal = -1; 
    var rowPick = -1;

    for (var i = 0; i < colA.length; i++) {
      var val = String(colA[i][0]);
      if (val.indexOf("총합") !== -1) rowTotal = i + 1; // '총합'이 포함된 행 (이미지상 23행 예상)
      if (val.indexOf("발표") !== -1 || val.indexOf("관계 없이") !== -1) rowPick = i + 1; // '발표' 관련 행 (이미지상 24행 예상)
    }
    
    // 혹시 A열 텍스트를 못 찾을 경우를 대비한 기본값 (이미지 기준)
    if (rowTotal === -1) rowTotal = 23; 
    if (rowPick === -1) rowPick = 24;

    var addedCount = 0;

    // 파일 리스트와 기존 팀을 비교하여 없는 팀만 추가
    fileList.forEach(function(fileInfo) {
      if (existingTeams.indexOf(fileInfo.name) == -1) {
        lastCol = sheet.getLastColumn(); 
        var newCol = lastCol + 1;
        
        // (1) B열(기준 서식) 복사 
        // -> 드롭다운, 테두리, 배경색 등 서식을 그대로 가져옵니다.
        var sourceRange = sheet.getRange(2, 2, lastRow - 1, 1);
        var destRange = sheet.getRange(2, newCol);
        sourceRange.copyTo(destRange); 
        
        // (2) 내용은 지우고 서식만 유지 (깨끗한 상태로 시작)
        sheet.getRange(2, newCol, lastRow - 1, 1).clearContent();

        // (3) 헤더: 팀명 추가 및 파일 링크 연결
        var headerCell = sheet.getRange(1, newCol);
        var rule = SpreadsheetApp.newRichTextValue()
          .setText(fileInfo.name)
          .setLinkUrl(fileInfo.url)
          .build();
        headerCell.setRichTextValue(rule);
        headerCell.setFontWeight("bold");
        headerCell.setHorizontalAlignment("center");
        headerCell.setBackground(null); 

        // (4) [핵심] 총점 수식 적용
        // 단순 숫자가 아닌, 각 항목의 'O' 개수를 세는 수식을 셀에 입력합니다.
        // 예: =COUNTIF(C3,"O") + COUNTIF(C4,"O") + ...
        var totalCell = sheet.getRange(rowTotal, newCol);
        
        var countFormulaParts = ROWS_TO_CHECK.map(function(r) {
          var cellAddr = sheet.getRange(r, newCol).getA1Notation();
          return 'COUNTIF(' + cellAddr + ', "O")';
        });
        
        var formulaString = "=" + countFormulaParts.join("+");
        
        totalCell.setFormula(formulaString); // 수식 입력
        totalCell.setBackground("#fff2cc");  // 노란색 배경 강조
        totalCell.setFontWeight("bold");
        totalCell.setHorizontalAlignment("center");
        totalCell.setNumberFormat("0");      // 정수 표시

        // (5) 발표 듣고 싶은가? (O/X 드롭다운 설정)
        var pickCell = sheet.getRange(rowPick, newCol);
        var ruleOX = SpreadsheetApp.newDataValidation()
          .requireValueInList(['O', 'X'], true)
          .setAllowInvalid(false)
          .build();
        pickCell.setDataValidation(ruleOX);
        pickCell.setHorizontalAlignment("center");

        addedCount++;
      }
    });
    
    if (addedCount > 0) {
      ss.toast(sheetName + " 시트에 " + addedCount + "개 팀 업데이트 완료!");
    }
  });
}
```
</details>

<details markdown="1">

<summary>2. 수정 버전 (v2.0)</summary>

<br>
단순히 점수 합산을 위한 시트가 아닌 '슈퍼패스'를 구분하기 위한 시트로 사용하고자 하였습니다<br>
점수의 총합이 높은 아이디어보다, 발표를 듣고싶은 신박한 아이디어에 더 기회를 주고자 하여 '슈퍼패스' 칸을 만들었습니다.<br>
또한 트리거를 사용하기 시작하였습니다. <br>

이때부터 코드가 크게 세가지였지만 가장 변화가 잦았던 3번만을 작성합니다. <br>
다른 코드는 현재 사용하는 것과 유사합니다.


```javascript

/**
 * 3. 최종 결과 집계 기능 (평가 시트)
 * - [디자인 수정] 순위표 제목(K1)을 L1, M1까지 병합하여 색칠
 * - [디자인 수정] 슈퍼패스 제목(O1)을 R1까지 병합하여 색칠
 * - 실행 팀 자동 초록색 표시 기능 포함
 */
function updateFinalDashboard() {
  var evaluators = ["평가자1", "평가자2", "평가자3"]; 
  var summarySheetName = "평가 시트"; 
  var SCORE_ROW = 23; 
  var VOTE_ROW = 24; 

  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(summarySheetName);

  if (!sheet) {
    ss.toast("❌ '" + summarySheetName + "' 시트가 없습니다.");
    return;
  }

  var fileList = getAllProjectFiles();
  if (fileList.length === 0) {
    ss.toast("📂 가져올 파일이 없습니다.");
    return;
  }

  // 초기화
  sheet.clear(); 
  
  // (A) 전체 데이터 헤더
  var headers = ["팀명", "링크", "평가자1(점)", "평가자2(점)", "평가자3(점)", "총점(30)", "평가자1(픽)", "평가자2(픽)", "평가자3(픽)", "구분"];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers])
    .setFontWeight("bold").setBackground("#f3f3f3").setHorizontalAlignment("center");

  // 데이터 채우기
  var startRow = 2;
  
  fileList.forEach(function(team, index) {
    var r = startRow + index;
    
    // 1. 팀명 & 링크
    sheet.getRange(r, 1).setValue(team.name);
    if (team.isExec) sheet.getRange(r, 1).setBackground("#d9ead3"); // 실행팀 배경색

    var linkRule = SpreadsheetApp.newRichTextValue().setText("이동").setLinkUrl(team.url).build();
    sheet.getRange(r, 2).setRichTextValue(linkRule).setHorizontalAlignment("center");

    // 2. 점수
    evaluators.forEach(function(evalName, i) {
      var formulaScore = "=IFERROR(HLOOKUP($A" + r + ", '" + evalName + "'!$1:$30, " + SCORE_ROW + ", 0), 0)";
      sheet.getRange(r, 3 + i).setFormula(formulaScore).setNumberFormat("0");
    });

    // 3. 총점
    sheet.getRange(r, 6).setFormula("=SUM(C" + r + ":E" + r + ")");

    // 4. 발표 픽
    evaluators.forEach(function(evalName, i) {
      var formulaVote = "=IFERROR(HLOOKUP($A" + r + ", '" + evalName + "'!$1:$30, " + VOTE_ROW + ", 0), \"X\")";
      sheet.getRange(r, 7 + i).setFormula(formulaVote).setHorizontalAlignment("center");
    });

    // 5. 실행팀 여부 마킹 (J열, 숨김 처리됨)
    if (team.isExec) {
      sheet.getRange(r, 10).setValue("EXEC");
    } else {
      sheet.getRange(r, 10).setValue("GEN");
    }
  });
  
  // 테두리 및 J열 숨기기
  var lastDataRow = startRow + fileList.length - 1;
  sheet.getRange(1, 1, lastDataRow, headers.length).setBorder(true, true, true, true, true, true);
  sheet.hideColumns(10); 

  // ==========================================
  // (B) 🏆 종합 순위표 (디자인 수정됨)
  // ==========================================
  var rankHeaderRow = 1;
  var rankColStart = 11; // K열

  // [수정] K1부터 M1까지(3칸) 합치고 색칠하기
  sheet.getRange(rankHeaderRow, rankColStart, 1, 3)
       .merge() // 셀 병합
       .setValue("🏆 종합 순위 (Top List)")
       .setFontWeight("bold").setFontSize(12)
       .setBackground("#fff2cc")
       .setHorizontalAlignment("center");
  
  sheet.getRange(rankHeaderRow + 1, rankColStart).setValue("순위");
  sheet.getRange(rankHeaderRow + 1, rankColStart + 1).setValue("팀명");
  sheet.getRange(rankHeaderRow + 1, rankColStart + 2).setValue("총점 (30)");

  var queryFormula = "=QUERY(A2:F, \"SELECT A, F WHERE A IS NOT NULL ORDER BY F DESC\", 0)";
  sheet.getRange(rankHeaderRow + 2, rankColStart + 1).setFormula(queryFormula);
  sheet.getRange(rankHeaderRow + 2, rankColStart).setFormula("=SEQUENCE(COUNTA(L3:L))");


  // ==========================================
  // (C) 🎫 슈퍼패스 (디자인 수정됨)
  // ==========================================
  var passColStart = 15; // O열

  // [수정] O1부터 R1까지(4칸) 합치고 색칠하기
  sheet.getRange(rankHeaderRow, passColStart, 1, 4)
       .merge() // 셀 병합
       .setValue("🎫 슈퍼패스 후보 (발표 O 있음)")
       .setFontWeight("bold").setFontSize(12)
       .setBackground("#d9ead3")
       .setHorizontalAlignment("center");

  var filterFormula = "=IFERROR(FILTER({A2:A, G2:I}, (G2:G=\"O\") + (H2:H=\"O\") + (I2:I=\"O\")), \"(발표 O 받은 팀 없음)\")";
  sheet.getRange(rankHeaderRow + 2, passColStart).setFormula(filterFormula);
  
  var passHeaders = ["팀명", "평가자1", "평가자2", "평가자3"];
  sheet.getRange(rankHeaderRow + 1, passColStart, 1, 4).setValues([passHeaders])
       .setFontWeight("bold").setBackground("#efefef").setHorizontalAlignment("center");


  // ==========================================
  // (D) 조건부 서식 (실행팀 초록색 표시)
  // ==========================================
  var rangeRankName = sheet.getRange("L4:L"); 
  var rangePassName = sheet.getRange("O4:O"); 
  
  var formula = '=VLOOKUP(L4, $A$2:$J, 10, 0)="EXEC"'; 
  var formulaPass = '=VLOOKUP(O4, $A$2:$J, 10, 0)="EXEC"';

  var ruleRank = SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied(formula)
    .setBackground("#d9ead3") 
    .setRanges([rangeRankName])
    .build();

  var rulePass = SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied(formulaPass)
    .setBackground("#d9ead3") 
    .setRanges([rangePassName])
    .build();

  var rules = sheet.getConditionalFormatRules();
  rules.push(ruleRank);
  rules.push(rulePass);
  sheet.setConditionalFormatRules(rules);

  ss.toast("✅ 디자인 수정 완료! (헤더 병합됨)");
}



```
</details>



3. 최종 버전 (v3.0)

두번째 버전도 굉장히 잘 사용하고 있었으나, 각 팀에 대해 메모해둔 내용이 페이지를 재실행하면 전부 사라지는 문제가 있어서 이를 수정하였습니다. 


```javascript
/**
 * 1. 메뉴 생성
 */
function onOpen() {
  var ui = SpreadsheetApp.getUi();
  ui.createMenu('🏆 공모전 평가 관리')
      .addItem('1. 팀 리스트 업데이트 (개별 시트)', 'updateEvaluationListCountO')
      .addItem('2. 최종 결과 집계 (평가 시트)', 'updateFinalDashboard')
      .addToUi();
}

/**
 * [공통 함수] 모든 폴더(일반+실행)에서 파일 목록 가져오기
 */
function getAllProjectFiles() {
  // ▼ 폴더 ID 설정 (본인의 폴더 ID로 수정되어 있는지 확인하세요)
  var GENERAL_FOLDER_ID = "아이디어 제출 파일 폴더 ID"; // 일반 팀 폴더
  var EXEC_FOLDER_ID = "제출 및 실행 파일 폴더 ID";    // 실행 팀 폴더

  var list = [];
  
  function scanFolder(folderId, isExec) {
    try {
      var folder = DriveApp.getFolderById(folderId);
      var files = folder.getFiles();
      while (files.hasNext()) {
        var file = files.next();
        list.push({ 
          name: file.getName(), 
          url: file.getUrl(), 
          isExec: isExec 
        });
      }
    } catch (e) {
      console.log("폴더 접속 실패: " + folderId);
    }
  }

  scanFolder(GENERAL_FOLDER_ID, false); 
  scanFolder(EXEC_FOLDER_ID, true);     

  return list;
}

/**
 * 2. 개별 평가 시트 업데이트
 */
function updateEvaluationListCountO() {
  var targetSheets = ["평가자1", "평가자2", "평가자3"]; 
  var ROWS_TO_CHECK = [3, 4, 5, 6, 10, 11, 12, 16, 17, 20]; 

  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var allFiles = getAllProjectFiles();

  if (allFiles.length === 0) {
    ss.toast("📂 가져올 파일이 없습니다. 폴더 ID를 확인하세요.");
    return;
  }

  targetSheets.forEach(function(sheetName) {
    var sheet = ss.getSheetByName(sheetName);
    if (!sheet) return;

    var lastRow = sheet.getLastRow();
    if (lastRow < 30) lastRow = 30; 

    // 기존 팀 확인
    var lastCol = sheet.getLastColumn();
    var existingTeams = [];
    if (lastCol > 1) {
      existingTeams = sheet.getRange(1, 2, 1, lastCol - 1).getValues()[0]; 
    }

    // 행 위치 찾기
    var colA = sheet.getRange(1, 1, lastRow, 1).getValues(); 
    var rowTotal = 23; 
    var rowPick = 24;

    for (var i = 0; i < colA.length; i++) {
      var val = String(colA[i][0]);
      if (val.indexOf("총합") !== -1 && val.indexOf("의견") === -1) rowTotal = i + 1;
      if (val.indexOf("발표") !== -1 || val.indexOf("관계 없이") !== -1) rowPick = i + 1;
    }

    var addedCount = 0;

    allFiles.forEach(function(fileInfo) {
      if (existingTeams.indexOf(fileInfo.name) == -1) {
        lastCol = sheet.getLastColumn(); 
        var newCol = lastCol + 1;
        
        var sourceRange = sheet.getRange(2, 2, lastRow - 1, 1);
        sourceRange.copyTo(sheet.getRange(2, newCol)); 
        
        sheet.getRange(2, newCol, lastRow - 1, 1).clearContent();

        var headerCell = sheet.getRange(1, newCol);
        var rule = SpreadsheetApp.newRichTextValue().setText(fileInfo.name).setLinkUrl(fileInfo.url).build();
        headerCell.setRichTextValue(rule).setFontWeight("bold").setHorizontalAlignment("center");
        
        // 실행 팀 배경색
        if (fileInfo.isExec) headerCell.setBackground("#d9ead3");
        else headerCell.setBackground(null);

        // 총점 수식
        var totalCell = sheet.getRange(rowTotal, newCol);
        var countFormulaParts = ROWS_TO_CHECK.map(function(r) {
          return 'COUNTIF(' + sheet.getRange(r, newCol).getA1Notation() + ', "O")';
        });
        totalCell.setFormula("=" + countFormulaParts.join("+"))
                 .setBackground("#fff2cc").setFontWeight("bold").setHorizontalAlignment("center");

        // 발표 OX
        var pickCell = sheet.getRange(rowPick, newCol);
        var ruleOX = SpreadsheetApp.newDataValidation().requireValueInList(['O', 'X'], true).build();
        pickCell.setDataValidation(ruleOX).setHorizontalAlignment("center");
        
        if (fileInfo.isExec) pickCell.setValue("O");
        else pickCell.setValue("");

        addedCount++;
      }
    });
    
    if (addedCount > 0) ss.toast(sheetName + " 시트: " + addedCount + "개 팀 추가 완료!");
  });
}

/**
 * 3. 최종 결과 집계 기능 (평가 시트)
 * - [수정] 시트 전체 초기화(clear) 제거 -> 필요한 부분만 지우도록 변경
 * - 이제 S열 옆에 적어둔 메모가 날아가지 않습니다.
 */
function updateFinalDashboard() {
  var evaluators = ["평가자1", "평가자2", "평가자3"]; 
  var summarySheetName = "평가 시트"; 
  var SCORE_ROW = 23; 
  var VOTE_ROW = 24; 

  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(summarySheetName);

  if (!sheet) {
    ss.toast("❌ '" + summarySheetName + "' 시트가 없습니다.");
    return;
  }

  var fileList = getAllProjectFiles();
  if (fileList.length === 0) {
    ss.toast("📂 가져올 파일이 없습니다.");
    return;
  }

  // ==========================================
  // [중요] 전체 삭제(sheet.clear()) 대신, 사용하는 열만 골라서 지웁니다.
  // 그래야 S열 이후에 적어둔 메모가 보존됩니다.
  // ==========================================
  var maxRows = sheet.getMaxRows();
  
  // 1. 전체 데이터 영역 지우기 (A열 ~ J열)
  sheet.getRange(1, 1, maxRows, 10).clear(); 
  
  // 2. 종합 순위표 영역 지우기 (K열 ~ M열)
  sheet.getRange(1, 11, maxRows, 3).clear();

  // 3. 슈퍼패스 영역 지우기 (O열 ~ R열)
  sheet.getRange(1, 15, maxRows, 4).clear();

  // 4. 조건부 서식 초기화 (규칙이 중복으로 쌓이는 것 방지)
  sheet.clearConditionalFormatRules();


  // ==========================================
  // (A) 전체 데이터 채우기
  // ==========================================
  var headers = ["팀명", "링크", "평가자1(점)", "평가자2(점)", "평가자3(점)", "총점(30)", "평가자1(픽)", "평가자2(픽)", "평가자3(픽)", "구분"];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers])
    .setFontWeight("bold").setBackground("#f3f3f3").setHorizontalAlignment("center");

  var startRow = 2;
  
  fileList.forEach(function(team, index) {
    var r = startRow + index;
    
    // 1. 팀명 & 링크
    sheet.getRange(r, 1).setValue(team.name);
    if (team.isExec) sheet.getRange(r, 1).setBackground("#d9ead3"); 

    var linkRule = SpreadsheetApp.newRichTextValue().setText("이동").setLinkUrl(team.url).build();
    sheet.getRange(r, 2).setRichTextValue(linkRule).setHorizontalAlignment("center");

    // 2. 점수
    evaluators.forEach(function(evalName, i) {
      var formulaScore = "=IFERROR(HLOOKUP($A" + r + ", '" + evalName + "'!$1:$30, " + SCORE_ROW + ", 0), 0)";
      sheet.getRange(r, 3 + i).setFormula(formulaScore).setNumberFormat("0");
    });

    // 3. 총점
    sheet.getRange(r, 6).setFormula("=SUM(C" + r + ":E" + r + ")");

    // 4. 발표 픽
    evaluators.forEach(function(evalName, i) {
      var formulaVote = "=IFERROR(HLOOKUP($A" + r + ", '" + evalName + "'!$1:$30, " + VOTE_ROW + ", 0), \"X\")";
      sheet.getRange(r, 7 + i).setFormula(formulaVote).setHorizontalAlignment("center");
    });

    // 5. 실행팀 여부 마킹 (J열)
    if (team.isExec) {
      sheet.getRange(r, 10).setValue("EXEC");
    } else {
      sheet.getRange(r, 10).setValue("GEN");
    }
  });
  
  // 테두리 및 J열 숨기기
  var lastDataRow = startRow + fileList.length - 1;
  sheet.getRange(1, 1, lastDataRow, headers.length).setBorder(true, true, true, true, true, true);
  sheet.hideColumns(10); 


  // ==========================================
  // (B) 🏆 종합 순위표
  // ==========================================
  var rankHeaderRow = 1;
  var rankColStart = 11; // K열

  sheet.getRange(rankHeaderRow, rankColStart, 1, 3)
       .merge()
       .setValue("🏆 종합 순위 (Top List)")
       .setFontWeight("bold").setFontSize(12)
       .setBackground("#fff2cc")
       .setHorizontalAlignment("center");
  
  sheet.getRange(rankHeaderRow + 1, rankColStart).setValue("순위");
  sheet.getRange(rankHeaderRow + 1, rankColStart + 1).setValue("팀명");
  sheet.getRange(rankHeaderRow + 1, rankColStart + 2).setValue("총점 (30)");

  var queryFormula = "=QUERY(A2:F, \"SELECT A, F WHERE A IS NOT NULL ORDER BY F DESC\", 0)";
  sheet.getRange(rankHeaderRow + 2, rankColStart + 1).setFormula(queryFormula);
  sheet.getRange(rankHeaderRow + 2, rankColStart).setFormula("=SEQUENCE(COUNTA(L3:L))");


  // ==========================================
  // (C) 🎫 슈퍼패스
  // ==========================================
  var passColStart = 15; // O열

  sheet.getRange(rankHeaderRow, passColStart, 1, 4)
       .merge()
       .setValue("🎫 슈퍼패스 후보 (발표 O 있음)")
       .setFontWeight("bold").setFontSize(12)
       .setBackground("#d9ead3")
       .setHorizontalAlignment("center");

  var filterFormula = "=IFERROR(FILTER({A2:A, G2:I}, (G2:G=\"O\") + (H2:H=\"O\") + (I2:I=\"O\")), \"(발표 O 받은 팀 없음)\")";
  sheet.getRange(rankHeaderRow + 2, passColStart).setFormula(filterFormula);
  
  var passHeaders = ["팀명", "김수아", "선서영", "홍은별"];
  sheet.getRange(rankHeaderRow + 1, passColStart, 1, 4).setValues([passHeaders])
       .setFontWeight("bold").setBackground("#efefef").setHorizontalAlignment("center");


  // ==========================================
  // (D) 조건부 서식
  // ==========================================
  var rangeRankName = sheet.getRange("L4:L"); 
  var rangePassName = sheet.getRange("O4:O"); 
  
  var formula = '=VLOOKUP(L4, $A$2:$J, 10, 0)="EXEC"'; 
  var formulaPass = '=VLOOKUP(O4, $A$2:$J, 10, 0)="EXEC"';

  var ruleRank = SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied(formula)
    .setBackground("#d9ead3") 
    .setRanges([rangeRankName])
    .build();

  var rulePass = SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied(formulaPass)
    .setBackground("#d9ead3") 
    .setRanges([rangePassName])
    .build();

  var rules = sheet.getConditionalFormatRules();
  rules.push(ruleRank);
  rules.push(rulePass);

  sheet.setConditionalFormatRules(rules);

  ss.toast("✅ 업데이트 완료! (수기 메모 보존됨)");
}

```

## 결론

공모전 결과 발표가 1월 중으로 끝나므로, 그 이전까지 요긴하게 사용하며 편의성이 떨어지는 부분은 계속 수정해 갈 생각입니다. 
